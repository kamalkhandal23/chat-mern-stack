...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    s.on('message', msg => {
      setMessages(prev => {
        ...
        if (exists) return prev.map(m => String(m._id) === String(msg._id) ? msg : m);
        return [...prev, msg];
      });
    ...
    s.on('message-deleted', ({ _id }) => {
      setMessages(prev => prev.map(m => String(m._id) === String(_id) ? { ...m, deleted: true } : m));
    });
    ...
    try {
      ...
      if (!list || list.length === 0) {
        ...
        setActiveRoom(prevActive => {
          if (prevActive) {
            const exists = list.some(r => String(r._id) === String(prevActive));
  ...
  async function sendMessage(e) {
    ...
    if (uploadPreview?.file) {
      try {
        ...
        console.error(err); alert('Upload failed'); setUploadPreview(null); return;
      }
    }
    ...
    s.emit('send-message', { roomId: activeRoom, text, attachments });
    setText('');
  ...
  const saveRename = async (roomId) => {
    ...
    try {
      ...
      const serverMsg = err?.response?.data?.message || err.message || 'Rename failed';
      alert('Rename failed: ' + serverMsg);
    }
  };
  ...
  const submitEdit = async e => {
    ...
    try {
      ...
      setMessages(prev => prev.map(m => String(m._id) === String(editingMsgId) ? res.data : m));
  ...
  const deleteMessage = async msg => {
    ...
    if (String(msg._id).startsWith('tmp-')) { setMessages(prev => prev.filter(m => String(m._id) !== String(msg._id))); return; }
    try {
      await API.delete(`/messages/${msg._id}`);
      setMessages(prev => prev.map(m => String(m._id) === String(msg._id) ? { ...m, deleted: true } : m));
    } catch (err) {
